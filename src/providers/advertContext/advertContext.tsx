...
export const AdvertProvider = ({ children }: IDefaultProviderProps) => {
  ...
  const { setLoading, getUserLoged, profile } = useContext(UserContext);
  ...
  async function createAdvert(formData: TAdvertRequest, image: File) {
    ...
    try {
      ...
      toast.success(`Anúncio ${response.data.model} cadastrado com sucesso `);
      setCreateAdvertModal(false);
    } catch (error: any) {
      if (error.response?.data.message.cover_image[0] == "Required") {
  ...
  async function updateAdvert(
    ...
    if (newCoverImage) {
      const newImage = { image: newCoverImage };
  ...
  function updateStatusAdverts() {
    if (profile) {
      if (profile?.is_advertiser == false) {
        try {
          setLoading(true);

          profile?.adverts.map((currentAdvert) => {
            if (currentAdvert?.status == true) {
              updateAdvert({ status: false }, currentAdvert!.id);
            }
          });
        } catch (error) {
          console.log(error);
  ...
  async function getSpeceficAdvert(id: number) {
    try {
      setLoading(true);
  ...
  async function getExternalCars() {
